...
def run_gcp_stage2_plan(
    ...
    *,
    ...
    configs_to_try = [c for c in candidate_configs if not (c in seen or seen.add(c))]
        ...
        for step in step_order:
            if step == "vm_create":
                ...
                for candidate_zone, candidate_gpu in configs_to_try:
                    alt_config = replace(current_plan.config, zone=candidate_zone, gpu_type=candidate_gpu)
                    ...
                    if console is not None:
                        console.print(f"[bold]Stage 2 cloud:[/bold] `vm_create` (zone: {candidate_zone}, gpu: {candidate_gpu})")
                    ...
                    if res.returncode == 0:
                        ...
                        if console is not None:
                            console.print(
                                f"[yellow]Zone `{candidate_zone}` / GPU `{candidate_gpu}` failed. Trying fallback candidate...[/yellow]"
                            )
                if not vm_created_successfully:
                    raise StageExecutionError(
                        f"Stage 2 cloud step `vm_create` failed across all candidate configs."
                    ) from last_exc
            else:
